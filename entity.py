"""
Entities for Subterranean Torment
"""
...
class Entity:
    ...
    def move_astar(self, target, entities, game_map):
        ...
        # scan the current map each turn and set all walls as blocking
        ...
        # also check that the object isnt self or the target (so that the start and endpoints are free)
        ...
        # the path size matters if you want the monster to use alternative longer paths (for example through other rooms) if the player is in a corridor
    ...
    def distance(self, x, y):
        """
        returns distance to a target tile
        """
        return math.sqrt((x - self.x) ** 2 + (y - self.y) **2)

    def distance_to(self, other):
        """
        returns distance to a target entity
        """
        dx = other.x - self.x
        dy = other.y - self.y
